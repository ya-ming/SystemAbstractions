 ...
 *
 * This module contains the unit tets of the SystemAbstractions:NetworkEndpoint class.
 */
...
namespace {
    ...
    /**
     * This is used to hold all the information about a received datagram that we care about.
     */
    ...
    struct Packet {
         ...
         *     This is the port number of the datagram sender.
         */
        Packet(const std::vector<uint8_t>& newPayload, uint32_t newAddress, uint16_t newPort)
            :payload(newPayload), address(newAddress), port(newPort)
        {
    ...
    struct Owner
    {
        ...
        /**
         * this flag indicates whether or not a connection to the network endpoint has been broken.
         */
...
/**
 * This is the test fixture for these tests, providing common setup and teardown for each test.
 */
...
TEST_F(NetworkEndpointTests, DatagramSending) {
    ...
    endpoint.Open(
        ...
        [&owner](
            uint32_t address, uint16_t port, const std::vector<uint8_t>& body) {
...
TEST_F(NetworkEndpointTests, DatagramReceiving) {
    ...
    // Test receiving a datagram from the unit under test.
    const std::vector<uint8_t> testPacket{ 0x12, 0x34, 0x56, 0x78 };
